...
def test_parser_simple_constraint_sampling():
    ...
    expected = Normal(Param(name="tau", index=None, lower=RealConstant(0.0), upper=RealConstant(float("inf"))), RealConstant(0.0), RealConstant(1.0))
...
def test_parser_complex_constraint_sampling():
    ...
    data_names = []
    statement = Parser(scanner(input_str), data_names).statement()
    expected = Normal(Param(name="tau", index=None, lower=Pow(Exp(RealConstant(0.0)), IntegerConstant(1)), upper=RealConstant(float("inf"))), RealConstant(0.0), RealConstant(1.0))
    assert statement.__str__() == expected.__str__()
...
def test_parser_rhs_index_shift():
    ...
    data_names = ["year"]
    statement = Parser(scanner(input_str), data_names).statement()
    expected = Normal(Param(name="tau", index=None, lower=RealConstant(0.0), upper=RealConstant(float("inf"))),
