    ...
    def __init__(self, draw_dfs: Dict[str, pandas.DataFrame], tolerance = 1e-2):
        ...
        for name, draw_df in draw_dfs.items():
            ...
            if len(group_columns) > 0:
                ...
                converged_df = (
                    draw_df
                    .merge(median_df, on = group_columns, how = "left")
                ...
                if any(absolute_differences > thresholds):
                    ...
                    raise Exception(f"For {name} [{values_string}], absolute differences [{differences_string}] exceed thresholds [{thresholds_string}]")
